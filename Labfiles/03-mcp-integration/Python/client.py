...
async def connect_to_server(exit_stack: AsyncExitStack):
    # MCP is used instead of plain custom function tools so tools live in a separate, reusable server process:
    # any MCP-compatible client/agent can call the same tools without redefining or duplicating their code,
    # and the tool implementations can change independently of the agent that uses them.
...
async def chat_loop(session):
    ...
    with (
        ...
        def make_tool_func(tool_name):
            # Without this factory, every tool_func would share the same outer tool_name variable and all end up calling whichever tool was assigned last
        ...
        agent = project_client.agents.create_version(
            ...
            definition=PromptAgentDefinition(
                ...
                instructions="""You are an inventory assistant. Here are some general guidelines:
